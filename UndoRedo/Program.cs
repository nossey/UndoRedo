...
namespace UndoRedo
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Use lamda expression to record your action.
            // Make sure that undo action is correct scheme.
            // If not, it results in invalid when you execute "Undo()."

            // Increment x by 1
            Record(() => { x++; }, () => { x--; });
            WriteLine($"{nameof(x)} is {x}.");

            // Undo the last action (means decrement by 1)
            Undo();
            WriteLine($"{nameof(x)} is {x}.");
            ...
            // Redo the action (means increment by 1)
            Redo();
            WriteLine($"{nameof(x)} is {x}.");
            ...
            /// Transaction example
            // Transaction packs multiple actions as one command.
